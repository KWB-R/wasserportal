...
## CensorCode: either "lt" (less than) for concentration below detection limit 
...
gwq_subs <- gwq_data %>%  
  ...
  dplyr::ungroup() %>% 
  tidyr::complete(Parameter, tidyr::nesting(CensorCode), 
                  fill = list(n = 0)) %>% 
  dplyr::group_by(.data$Parameter) %>% 
  tidyr::pivot_wider(names_from = CensorCode, values_from = n) %>% 
  dplyr::mutate(n_total = lt + nc, 
                percent_nc = 100*nc/n_total) %>% 
  dplyr::rename(n_lt = lt, 
                n_nc = nc) %>% 
  dplyr::ungroup() %>% 
  dplyr::left_join(cas_wasserportal[, c("category", "category_name", "Parameter", "name", "cas_number")]) %>%
  dplyr::rename(name_wasserportal = Parameter, 
                name_uba = name) %>% 
  dplyr::select(category, category_name, name_wasserportal, name_uba, cas_number,n_lt, n_nc, n_total, percent_nc)
...
gwq_subs_stations <- gwq_data %>%  
  ...
  dplyr::rename(name_wasserportal = Parameter, 
                name_uba = name) %>% 
  dplyr::select(category, category_name, name_wasserportal, name_uba, cas_number, Messstellennummer, n_lt, n_nc, n_total, percent_nc) %>% 
  dplyr::left_join(gwq_master, by = c(Messstellennummer = "Nummer"))

 readr::write_csv( gwq_subs_stations, "gwq_subs_stations.csv")


 to_wide <- function(df) {
   
   df %>% 
   dplyr::select(-share_percent) %>% 
     tidyr::pivot_wider(names_from = "CensorCode", values_from = "n") %>% 
   dplyr::mutate()
 }
 
 gwq_subs_wide <- to_wide(gwq_subs_stations)
 gwq_subs_stations_wide <- to_wide(gwq_subs_stations)
