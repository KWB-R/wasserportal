...
DT::datatable(
  stations$crosstable, 
  filter = "top", 
  caption = "Data availabilty per monitoring station"
)
```
...
cat(paste0(
  ...
  "stations is also available in JSON format here: ",
...
cat(paste0(
  "The master data of GW level stations is also available in JSON format ", 
  "here: [https://kwb-r.github.io/wasserportal/stations_gwl_master.json]",
  "(../stations_gwl_master.json)"
))
```
...
percental_share_low_levels <- 100 * sum(
  gwl_classified_only$Klassifikation %in% text_low_levels
) / nrow(gwl_classified_only)

percental_share_high_levels <- 100 * sum(
  gwl_classified_only$Klassifikation %in% text_high_levels
) / nrow(gwl_classified_only) 
...
`r nrow(gwl_classified_only)` GW level monitoring stations containing
`classification` data (out of `r nrow(gwl)` provided by SenWeb) indicate `below
normal` (`r paste(text_low_levels, collapse = ", ")`) GW levels. However, only 
`r round(percental_share_low_levels, 2L)` percent are indicate `above normal` 
(`r paste(text_high_levels, collapse = ", ")`) GW levels.
...
cat(paste0(
  ...
  "[https://kwb-r.github.io/wasserportal/map_gwl-trend.html]", 
  "(../map_gwl-trend.html)"
))
```
...
title_subtitle <- paste0(
  paste0(names(station_gwl)[1], ": ", station_gwl[1], collapse = ", "),
  "<br>",
  "<sup>",
  paste0(names(station_gwl)[ncols], ": ", station_gwl[ncols], collapse = ", "),
  "</sup>"
)
...
cat(paste0(
  "The master data of GW quality stations is also available in JSON format ", 
  "here: [https://kwb-r.github.io/wasserportal/stations_gwq_master.json]",
  "(../stations_gwq_master.json)"
))
```
...
title_subtitle <- paste0(
  paste0(names(station_gwq)[1], ": ", station_gwq[1], collapse = ", "),
  "<br>",
  "<sup>",
  paste0(names(station_gwq)[ncols], ": ", station_gwq[ncols], collapse =", "),
  "</sup>"
)
...
cat(paste0(
  "The data of all GW quality stations is also available in CSV format here: ",
  "[https://kwb-r.github.io/wasserportal/groundwater_quality.csv]", 
