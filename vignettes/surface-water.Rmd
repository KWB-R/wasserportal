...
sw_data_to_json <- function(sw_data_daily_list) {
tmp <- lapply(seq_len(length(sw_data_daily_list)), function(i) {
  filename <- sprintf("daily_%s.json", names(sw_data_daily_list[i]))
  msg <- sprintf("Writting '%s'", filename)
                                                                          
  kwb.utils::catAndRun(messageText = msg,
                       expr = {
  jsonlite::write_json(sw_data_daily_list[[i]], 
                       path = filename
  )
                       })
  filename
...
files <- sw_data_to_json(sw_data_daily_list)
...
```
The following `.json` files are available for download: 

```{r daily_surface_water_data_json, echo = FALSE, results ='asis'}
...
```
- [%s](https://kwb-r.github.io/wasserportal/%s)
...
swl_data <- sw_data_daily$surface_water.water_level %>% 
