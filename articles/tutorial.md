...
#>   ..$ groundwater.level                 : tibble [894 × 11] (S3: tbl_df/tbl/data.frame)
#>   ..$ groundwater.quality               : tibble [208 × 10] (S3: tbl_df/tbl/data.frame)
#>  $ crosstable   : tibble [1,117 × 12] (S3: tbl_df/tbl/data.frame)
```
...
#> Importing master data for 894 stations from Wasserportal Berlin ... ok. (9.52 mins)
```
...
84.2 percent of all considered 810 groundwater level monitoring stations
containing `classification` data (out of 894 provided by SenWeb)
indicate `below normal` (extrem niedrig, sehr niedrig, niedrig)
groundwater levels. However, only 84.2 percent are indicate
...
#> Importing master data for 208 stations from Wasserportal Berlin ... ok. (2.57 mins)
```
