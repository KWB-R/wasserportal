...
#> Rows: 109631 Columns: 68
#> ── Column specification ────────────────────────────────────────────────────────
#> Delimiter: ","
#> chr (41): Norman_SusDat_ID, Name, Name_Dashboard, Name_ChemSpider, Name_IUPA...
#> dbl (27): PubChem_CID, Monoiso_Mass, M+H+, M-H-, Pred_RTI_Positive_ESI, Pred...
#> 
...
#> ── Column specification
#> ──────────────────────────────────────────────────────── Delimiter: "," chr
#> (11): CAS, Name, DTXSID, InChIKey, SMILES, InChI, MolecularFormula, IUPA... dbl
#> (2): PubChem_CID, MonoisotopicMass
...
stringr::str_remove(.data\$CAS_RN_Dashboard, pattern = "^CAS_RN:\\s"))
%\>% dplyr::filter(!is.na(.data\$cas_number), .data\$cas_number %in%
unique(cas_wasserportal\$cas_number))
...
samples % dplyr::rename(name_norman = .data\$Name_Dashboard) %\>%
dplyr::select(name_norman, cas_number, Messstellennummer, Datum,
...
.data\$Messstellennummer) %\>% \# dplyr::summarise(n_lt = sum(n_lt), \#
n_nc = sum(n_nc), \# n_total = sum(n_total)) %\>% \#
...
= "Nummer")) \# gwq_subs_stations_n_paras_abovedetection_wide % \#
dplyr::ungroup() %\>% \# dplyr::select(Messstellennummer, category,
n_paras_abovedetection) %\>% \# tidyr::pivot_wider(names_from =
"category", \# names_prefix = "cat\_", \# values_from =
"n_paras_abovedetection") %\>% \# dplyr::left_join(gwq_master, by =
c("Messstellennummer" = "Nummer")) samples_by_para_and_station_n %
dplyr::group_by(name_norman, cas_number) %\>%
dplyr::summarise(n_stations_sampled = dplyr::n(), n_stations_total =
length(unique(gwq_master\$Nummer)), n_lt = sum(n_lt), n_nc = sum(n_nc),
n_total = sum(n_total)) %\>%
...
%\>% dplyr::select(name_norman, cas_number, n_stations_abovedetection,
n_stations_sampled, n_stations_total, percent_stations_abovedetection,
percent_stations_sampled, n_belowdetection, n_abovedetection, n_total,
...
 \[1m \[22m\`summarise()\` has grouped output by 'name_norman'. You can
override using the \#\> \`.groups\` argument. \#\> Joining, by =
"cas_number" \### Export data to EXCEL gwq_data_list %
...
"wasserportal_gwq_norman-list_s0_v1.0.0.xlsx", overwrite = TRUE)

## Norman Substances in Wasserportal

Filter criteria: - log Kow (column `logKow_EPISuite` \<= `4.5`) -
...
![](promisces_norman-lists_files/figure-html/unnamed-chunk-5-1.png)
